...
pub struct Nursery {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub all: Option<bool>,
    #[doc = "Disallow anonymous default exports."]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub no_anonymous_default_export: Option<RuleConfiguration<NoAnonymousDefaultExport>>,
...
impl Nursery {
    ...
    pub(crate) fn get_enabled_rules(&self) -> IndexSet<RuleFilter> {
        let mut index_set = IndexSet::new();
        if let Some(rule) = self.no_anonymous_default_export.as_ref() {
            if rule.is_enabled() {
                index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[0]));
            }
        }
